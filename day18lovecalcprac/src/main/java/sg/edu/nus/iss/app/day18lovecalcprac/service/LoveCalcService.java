package sg.edu.nus.iss.app.day18lovecalcprac.service;
...
public class LoveCalcService {
    ...
    public Calculator[] findAll() throws IOException {
        Set<String> allMatchMakingdKeys = redisTemplate.keys("*");
        List<Calculator> mArr = new LinkedList<>();
        for (String matchMakeKey : allMatchMakingdKeys) {
            String result = (String) redisTemplate.opsForValue().get(matchMakeKey);

            mArr.add(Calculator.create(result));
        }

        return mArr.toArray(new Calculator[mArr.size()]);
    }
    ...
    public Optional<Calculator> getResult(String sname, String fname) throws IOException, InterruptedException {
        ...
        if (c != null){
            //to save data for Value Operations
